...
describe("parseArgs", () => {
    ...
    it("exits 2 for malformed --since", () => {
        expect(() => parseArgs(["node", "script", "/repo", "--since", "2x"])).toThrow("process.exit called");
        expect(process.exit).toHaveBeenCalledWith(2);
        expect(process.stderr.write).toHaveBeenCalledWith(expect.stringContaining("--since must be"));
    });
});
describe("parseSince", () => {
    it("parses days", () => {
