...
const fixture = readFileSync(resolve(__dirname, "../../test/fixtures/gitLog.txt"), "utf8");
describe("parseGitLog", () => {
    it("parses the three-commit fixture correctly", () => {
        const commits = parseGitLog(fixture);
        expect(commits).toHaveLength(3);
        expect(commits[0].sha).toBe("abc1234567890abcdef1234567890abcdef12345");
        expect(commits[0].subject).toBe("feat: add user authentication");
        expect(commits[0].body).toBe("TIL: JWT tokens need to be rotated periodically");
        expect(commits[0].author).toBe("Jane Dev");
        expect(commits[0].timestamp).toEqual(new Date("2026-03-30 10:00:00 +0000"));
        expect(commits[1].sha).toBe("def4567890abcdef1234567890abcdef12345678");
        expect(commits[1].subject).toBe("fix(parser): handle edge case in date parsing");
        expect(commits[1].author).toBe("John Coder");
        expect(commits[2].sha).toBe("789abcdef1234567890abcdef1234567890abcde");
    ...
    it("parses multi-line body correctly", () => {
        const commits = parseGitLog(fixture);
        const commit = commits[1];
        expect(commit.body).toBe("Fixed a bug where dates with single-digit months were not parsed correctly.");
    });
    ...
    it("parses body in subject-only commit (body is TIL line)", () => {
        ...
        expect(commit.body).toBe("TIL: JWT tokens need to be rotated periodically");
    });
