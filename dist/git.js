...
export const parseGitLog = (raw) => {
    ...
    return blocks.map((block) => {
        ...
        if (afterBodyLines.length < 2) {
            throw new Error(`Malformed commit block: missing author or timestamp for SHA ${sha}`);
        }
        const author = afterBodyLines[0].trim();
        const rawTimestamp = afterBodyLines[1].trim();
        const timestamp = new Date(rawTimestamp);
        if (isNaN(timestamp.getTime())) {
            throw new Error(`Malformed commit block: invalid timestamp "${rawTimestamp}" for SHA ${sha}`);
        }
